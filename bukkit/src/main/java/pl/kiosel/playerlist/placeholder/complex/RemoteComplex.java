...
public class RemoteComplex implements ComplexPlaceholder, Tickable {
    ...
    public RemoteComplex(ConfigurationSection sec) {
        ...
        server = sec.isList("server")
                ? sec.getStringList("server")
                : Arrays.asList(sec.getString("server", ""));
    ...
    public void run() {
        ...
        if (tickTime() % refreshTicks == 0) {
            ...
            if (bridge != null && bridge.isEnabled()) {
                for (String target : server) {
                    if (target != null && !target.trim().isEmpty()) {
                        bridge.requestRemote(this, target, handler);
                    }
                }
            }
        }
    ...
    public void update(String serverName, List<BridgeMessages.RemoteLine> remoteLines) {
        ExtraData[] data = new ExtraData[remoteLines.size()];
        for (int index = 0; index < remoteLines.size(); index++) {
            BridgeMessages.RemoteLine line = remoteLines.get(index);
            data[index] = new ExtraData()
                    .put(ExtraData.DATA_TEXT, line.getText())
                    .put(ExtraData.DATA_PING, line.getPing())
                    .put(ExtraData.DATA_SKIN, line.getSkin())
                    .put(ExtraData.DATA_OPACITY, line.getOpacity());
        }
        lines.put(serverName, data);
    }
            ...
            for (Map.Entry<String, ExtraData[]> entry : RemoteComplex.this.lines.entrySet()) {
                for (ExtraData d : entry.getValue()) {
                    if (d != null) {
                        ...
                        clone.put(ExtraData.DATA_OPACITY, replace(d.get(ExtraData.DATA_OPACITY)));
                        this.lin.add(clone);
                    }
                }
            }
